...
//        LOOP KULLANARAK DATALARI TEK TEK TEST CASEDE KULLANALIM
        for (Map<String, String> data : excelDatalari) {
            login();//Her Loopda Login Sayfasina Goturecek
//            kullanici adini gir
            ReusableMethods.waitFor(1);
            blueRentalLoginPage.emailBox.sendKeys(data.get("username"));  //BURDA EACH LOOPTAN GELEN W YANİ data "username" , key ini sırayla çağırıyor
//            kullanici sifresini gir
            ReusableMethods.waitFor(1);
            blueRentalLoginPage.passwordBox.sendKeys(data.get("password")); //BURDA EACH LOOPTAN GELEN W YANİ data "password" sırayla çağırıyor
//            login buttonuna tikla
            ReusableMethods.waitFor(1);
            blueRentalLoginPage.loginButton.click();
            ReusableMethods.waitFor(1);
            ReusableMethods.verifyElementDisplayed(blueRentalHomePage.userID);  //doğrulama
            ReusableMethods.waitFor(1);
